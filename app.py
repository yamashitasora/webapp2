...
def inverse_transform_feature(scaler, data_scaled, feature_index):
    data_scaled = np.atleast_1d(data_scaled)
    n_features = scaler.n_features_in_
    temp = np.zeros((len(data_scaled), n_features))
    temp[:, feature_index] = data_scaled
    data_original = scaler.inverse_transform(temp)
...
# --- 記念日CSVパス ---
csv_path = "C:/Users/ktdi104-alt/webapp/streamlit_project/data/anniversary_365_summary.csv"
anniversaries = load_anniversaries_from_csv(csv_path)
today_key = f"{now.month:02d}-{now.day:02d}"

# --- event_textを生成 ---
if today_key in anniversaries:
    events = anniversaries[today_key]
    events_sorted = sorted(events, key=lambda x: x['summary'], reverse=True)
...
def create_attention_lstm_model():
    ...
    x = Dropout(0.29421111209669787)(x)
    x = AttentionLayer()(x)
    x = Dropout(0.3)(x)
